...
> **Pre-1.0 状态。** `ssh-mcp` 处于 `0.x.y` 阶段。功能对个人 / 小团队场景已足够稳定（威胁模型见 `SECURITY.md`），但**当安全或设计修复需要时**，相邻 minor 版本之间仍可能出现破坏性变更 —— 每一处都会在 `CHANGELOG.md` 用 `### Breaking` 小节明确标注。`1.0.0` 发布起会承诺标准的 semver 兼容性。
