...
func printInstallUsage() {
	fmt.Fprintln(os.Stderr, "usage: ssh-mcp install <target>")
...
func installClaudeDesktop(exePath string) int {
	...
	fmt.Printf("# Edit: %s\n", configPath)
	fmt.Println("# WARNING: Do not add autoApprove for any of ssh-mcp tools.")
	fmt.Println()
	fmt.Println("{")
...
func defaultBinaryPath() string {
	switch runtime.GOOS {
	case "windows":
		return os.Getenv("LOCALAPPDATA") + `\Programs\ssh-mcp\ssh-mcp.exe`
	default:
		home, _ := os.UserHomeDir()
		return home + "/.local/bin/ssh-mcp"
	}
}
